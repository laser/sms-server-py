...
var UTM = function(latitude, longitude) {
    var self,
        xy,
        zone;
    ...
    self.lngZone = LatLonToUTMXY(DegToRad(latitude), DegToRad(longitude), zone, xy);
    self.easting = xy[0];
    self.northing = xy[1];
    self.hemisphere = latitude < 0 ? "S" : "N";

    return self;
};

var LL = function(easting, northing, zone, hemisphere) {
    var latlon,
        self;
    ...
    UTMXYToLatLon(parseFloat(easting), parseFloat(northing), parseFloat(zone), (hemisphere === "S"), latlon);

    self.longitude = RadToDeg(latlon[1]);
    self.latitude  = RadToDeg(latlon[0]);
